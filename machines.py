...
class BayesMachine(object):
    ...
    def __init__(self, df, clsfn):
        ...
        self.givnPDFs = dict([(clsf, dict(
            [(var, PDF.make(df[clsfr == clsf][var], start=df[var].min(), stop=df[var].max())) for var in self.varsn]))
            for clsf in self.clsfn])

    def prob(self, data):
        given = np.array([np.prod([self.givnPDFs[clsf][var].prob(data[var]) for var in data.keys()]) for clsf in self.clsfn])
        clsfs = np.array([self.clsfPDF.prob(clsf) for clsf in self.clsfn])
        prior = np.array([np.prod([self.varsPDFs[var].prob(data[var]) for var in data.keys()]) for clsf in self.clsfn])
        v = ((given * clsfs) / prior)
        return pd.DataFrame(columns=[str(clsf) for clsf in self.clsfn], data=[v * (1 / float(v.sum()))])
    ...
    def test(self, series, n=20, its=1000):
        ...
        def tester(series, its, n):
            ...
            cols.remove(name)
            naiv = np.array([self.prob({})[str(c)] for c in self.clsfn]).cumsum()
            while its:
